...
from common import ld_utils as ld_utils
from common import thing as thing
...
def prepare_data(full_seq_name, exp_key, data_keys, layers, device, task, eval_p, data_loader=None, model=None):
    sid = full_seq_name[:3]
    ...
    batch = read_keys(gt_folder, folder_p, keys=data_keys, verbose=False, data_loader=data_loader, model=model)
    batch = xdict(batch)
    logger.info("Done")
...
def read_keys(gt_folder_p, folder_p, keys, verbose=True, data_loader=None, model=None):
    out = {}

    idx = 1400
    samples, targets = [data for idx, data in enumerate(data_loader) if idx<1][0]
    pred = model.cuda()(samples.to('cuda'))
    ...
    from tool.wraper import process_data
    for key in pbar:
