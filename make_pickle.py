...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='./weights/FPHA_aug45/44.pth', help='resume from checkpoint')
    parser.add_argument('--start_epoch', default=0, type=int, metavar='N',
                        help='start epoch')
    parser.add_argument('--num_workers', default=8, type=int)
    parser.add_argument('--cache_mode', default=False, action='store_true', help='whether to cache images on memory')
    parser.add_argument('--img_size', default=(960, 540), type=tuple)
    parser.add_argument('--make_pickle', default=True, action='store_true')
...
def main(args):
    ...
    # for i in range(50):
    #     idx = 49-i
    #     weights_path = f'./weights/FPHA/{idx}.pth'
    #     checkpoint = torch.load(weights_path, map_location='cpu')
    #     missing_keys, unexpected_keys = model_without_ddp.load_state_dict(checkpoint['model'], strict=False)
    #     header = 'Epoch: [{}]'.format(idx)
    #     print(header)
    #     val_stats=test_pose(model, criterion, data_loader_val, device, cfg, vis=False, save_pickle=False)
    #     print(f"Val ||left : {val_stats['left']} || right : {val_stats['right']} || obj : {val_stats['obj']} || class : {val_stats['class_error']}")
        
    # print("Start training")
    # start_time = time.time()

    # train_stats = test_pose(model, criterion, data_loader_train, device, cfg, vis=False, save_pickle=True)
    # print(f"Train ||left : {train_stats['left']} || right : {train_stats['right']} || obj : {train_stats['obj']} || class : {train_stats['class_error']}")

    val_stats = test_pose(model, criterion, data_loader_val, device, cfg, vis=False, save_pickle=True)
    print(f"Val ||left : {val_stats['left']} || right : {val_stats['right']} || obj : {val_stats['obj']} || class : {val_stats['class_error']}")
        
    if args.dataset_file == 'H2O':
        test_stats = test_pose(model, criterion, data_loader_test, device, cfg, vis=False, save_pickle=True)
