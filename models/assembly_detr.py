...
from .assembly_transformer import build_deforamble_transformer, _get_activation_fn
import copy
import numpy as np
...
class SetAssemblyCriterion(nn.Module):
...
def build(args, cfg):
    ...
    criterion = SetAssemblyCriterion(num_classes, matcher, weight_dict, losses, focal_alpha=args.focal_alpha, cfg=cfg)
    criterion.to(device)
